...
public class FileUtility
{
    ...
    public static string GetStringCurrencyFormat(int origin)
    {
        return origin.ToString("N0");
    }
    ...
    public static void DrawCircle(Vector2 start, float radius, Color color, float time = 2)
    {
        int N = 50;
        float x = start.x - radius;
        
        Vector3 tstart = new Vector3(0,0,0);
        Vector3 tend = new Vector3(0,0,0);
        
        for (int i = 0; i < N; i++)
        {
            float x1 = x + i * (2f * radius / N);
            float x2 = x + (i + 1) * (2f * radius / N);
            tstart.x = x1;
            tstart.y = start.y + Mathf.Sqrt(radius * radius - Mathf.Pow(x1 - start.x, 2));
            tend.x = x2;
            tend.y = start.y + Mathf.Sqrt(radius * radius - Mathf.Pow(x2 - start.x, 2));
            
            Debug.DrawLine(tstart,tend, color ,time);
    ...
    public static void SetSpriteMaterialFloat(SpriteRenderer render , string valueName , float value)
    {
        var material = render.material;
        material.SetFloat(valueName , value);
    }

    public static void SetImageMaterialFloat(Image render, string valueName, float value)
    {
        var material = render.material;
        material.SetFloat(valueName, value);
    }
