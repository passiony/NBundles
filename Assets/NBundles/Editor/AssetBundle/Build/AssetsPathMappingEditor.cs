...
namespace AssetBundles
{
    public class AssetsPathMappingEditor
    {
        ...
        public static bool BuildPathMapping(AssetBundleManifest manifest)
        {
            mappingList.Clear();
            ...
            foreach (string assetbundle in assetbundlesWithoutVariant)
            {
                ...
                if (assetbundle == "lua.assetbundle" && assetPaths.Length<3)
                {
                    return false;
                }
            ...
            foreach (string assetbundle in variantWithoutDeplicate)
            {
                ...
                string nowRoot = FileUtility.FormatToUnityPath(System.IO.Path.GetDirectoryName(assetbundlePath));
                foreach (string assetPath in assetPaths)
                {
            ...
            if (!FileUtility.SafeWriteAllLines(outputFilePath, mappingList.ToArray()))
            {
